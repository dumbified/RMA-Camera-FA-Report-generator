...
def _load_customer_request_templates() -> Dict[str, Any]:
    ...
    here = os.path.dirname(__file__)
    # Stored alongside this migration script for easier editing and maintainability.
    path = os.path.abspath(os.path.join(here, "customer_request_templates.json"))
...
def migrate() -> None:
    ...
    try:
        ...
        #   report_context.action_taken.<sub_key> -> each action_taken section in its own row
        report_ctx = rules_json.get("report_context", {})
        if report_ctx:
            for top_key, top_value in report_ctx.items():
                if top_key == "action_taken" and isinstance(top_value, dict):
                    for sub_key, sub_val in top_value.items():
                        cursor.execute(
                            f"""
                            INSERT INTO `{data_table}`
                            ...
                            VALUES ('context', %s, %s)
                            """,
                            (f"report_context.action_taken.{sub_key}", json.dumps(sub_val)),
                        )
                else:
                    cursor.execute(
                        f"""
